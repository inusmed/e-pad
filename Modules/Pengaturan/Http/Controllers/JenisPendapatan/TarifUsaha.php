...
class TarifUsaha extends Controller
{
    ...
    {        
        return view('pengaturan::jenis-pendapatan.tarif-usaha.index');
    }
    ...
    public function create()
    {
        ...
        return view('pengaturan::jenis-pendapatan.tarif-usaha.create', compact(
            'company_id',
            'opttarifGrup',
            'optreffPeriodeTarif'
        ));
    ...
    public function edit($company_id, $uuid)
    {
        ...
        return view('pengaturan::jenis-pendapatan.tarif-usaha.edit', compact(
            'company_id',
            'uuid',
            'opttarifGrup',
            'optreffPeriodeTarif'
            ...
            if( request()->has('grup_id') &&  !is_null(request('grup_id')) && request()->has('kategori_id') &&  !is_null(request('kategori_id')) ) 
            {
                if( request()->has('subkategori_id') &&  !is_null(request('subkategori_id')) && request()->has('subrekening_id') &&  !is_null(request('subrekening_id')) ) 
                {
                    if( request()->has('rekening_id') &&  !is_null(request('rekening_id')) && request()->has('pendapatan_id') &&  !is_null(request('pendapatan_id')) )
                    {
                        ...
                        return view('pengaturan::jenis-pendapatan.tarif-usaha.filter', compact(
